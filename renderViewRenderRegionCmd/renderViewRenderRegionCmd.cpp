...
// DESCRIPTION:
// 
// Produces the MEL command "renderViewRenderRegion".
//
// This example demonstrates how to use the MRenderView class to update
// the currently selected Render Region in Maya's Render View.
// The command takes no arguments and always updates the Render Region
// with a blue and white circular pattern. The command assumes that the Render
// View is currently displaying a 640x480 image (such as the one generated by
// the renderViewRender example command).
//
...
public:                 
    ...
    ~renderViewRenderRegion() override {};
