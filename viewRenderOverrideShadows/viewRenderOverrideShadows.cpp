...
MStatus viewRenderOverrideShadows::updateLightList()
{
    ...
    for (unsigned int i=0; i<selectList.length(); i++)
    {
        ...
        // Compare with directional light classification to check if this node present a directional light.
        MString directionalLightClassification = MString("drawdb/light/directionalLight");
        int result = memcmp(drawdb.asWChar(), directionalLightClassification.asWChar(), directionalLightClassification.numChars()*sizeof(wchar_t));

        // Prune by directionalLight classification.
        if ( result==0 )
        {
